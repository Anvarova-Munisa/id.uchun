...
from keyboards.inlainKeyboard.tugmaKeyboard import tugma,car,rang,tasdiqlash,cars
...
async def tanlandi1(callback: types.CallbackQuery):
    ...
    narx = cars.get(car_name)
    await callback.answer(f"{car_name.capitalize()} nasib qilsin!")
    await callback.message.edit_text(f"siz {car_name.capitalize()} tanladingiz. Narxi: {narx:,} so'm \n\nRangni tanlang", reply_markup=rang())
...
async def rang7(callback: types.CallbackQuery):
...
async def ha(callback: types.CallbackQuery):
    await callback.answer()
...
async def yoq(callback: types.CallbackQuery):
    await callback.answer()
    await callback.message.edit_text("Xo'p")


@router.callback_query(F.data == "senga nma")
async def qiziqib(callback: types.CallbackQuery):
    await callback.answer()
